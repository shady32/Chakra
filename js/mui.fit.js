...
(function($, window, document, name) {
	...
	var onPopoverShown = function(e) {
		...
		document.body.classList.add("mui-popover-active-for-page");
	}
	var onPopoverHidden = function(e) {
		...
		document.body.classList.remove("mui-popover-active-for-page");
	};
	...
	var removeBackdrop = function(popover) {
		...
		removeBackdropTimer = $.later(function() {
			if(!popover.classList.contains(CLASS_ACTIVE)) {
				document.body.querySelector(".page-current").removeChild(backdrop);
			}
		}, 350);
	};
	window.addEventListener('tap', function(e) {
	...
	var togglePopover = function(popover, anchor, state) {
		...
		document.body.querySelector(".page-current").appendChild(backdrop);
	...
	$.createMask = function(callback) {
		...
		mask.show = function() {
			...
			document.body.querySelector(".page-current").appendChild(element);
			return mask;
		};
		mask._remove = function() {
			if(mask._show) {
				...
				$.later(function() {
					document.body.querySelector(".page-current").removeChild(element);
					//var body = document.body;
...
(function($, window, document) {
	...
	var createPopup = function(html, callback) {
		...
		document.body.querySelector(".page-current").appendChild(popupElement);
		document.body.classList.add("mui-popup-active-for-page");
		...
		var popup = {
			...
			close: function(index, animate) {
				if(popupElement) {
					document.body.classList.remove("mui-popup-active-for-page");
					callback && callback({
						index: index || 0,
	...
	var createPrompt = function(message, placeholder, title, btnArray, callback, type, value) {
		if(typeof message === 'undefined') {
			return;
		} else {
		...
		if(!$.os.plus || type === 'div') {
			return createPopup(createInner(message, title || '提示', createInput(placeholder, value)) + createButtons(btnArray || ['取消', '确认']), callback);
		}
...
(function($) {
	...
	$.fn.numbox = function(options) {
		...
		this.each(function(i, element) {
			if(element.numbox) {
				element.numbox.setOptions(options);
				return;
			}
			if(options) {
				...
				var option = optionsText ? JSON.parse(optionsText) : {};
				option.step = element.getAttribute('data-numbox-step') || options.step;
				option.min = element.getAttribute('data-numbox-min') || options.min;
				option.max = element.getAttribute('data-numbox-max') || options.max;
				element.numbox = new Numbox(element, option);
			}
		});
...
(function($, document) {
	...
	var PopPicker = $.PopPicker = $.Class.extend({
		...
		init: function(options) {
			...
			document.body.querySelector(".page-current").appendChild(self.panel);
...
(function($, document) {
	...
	var DtPicker = $.DtPicker = $.Class.extend({
		init: function(options) {
			...
			document.body.querySelector(".page-current").appendChild(_picker);
